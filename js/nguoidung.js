        ...
        if (isPaid(orderCode)) {
            ...
            <tr>
                ...
                <td class="noPadding imgHide">
                    <a target="_blank" href="qrveuser.html?` + p.name.split(' ').join('-') + `" title="Xem chi tiết">
                        ` + p.name + `
                        <img src="` + p.img + `">
                    </a>
                </td>
...
async function fetchPaidList() {
    try {
        const response = await fetch('http://localhost:8080/payment/getPaidList');
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
        paidOrders = data; // Lưu kết quả vào biến toàn cục
    } catch (error) {
...
function isPaid(value) {
    ...
    return paidOrders.includes(value);
}